...
body {
    ...
    user-select: none;         /* Standard */
        ...
        .chat-bubble.sent {
            /* 修改：使用新的CSS变量，并提供默认值 */
            background-color: var(--my-bubble-color, var(--accent-color));
            color: white;
            border-radius: 22px;
...
.custom-alert-box .modal-button {
    /* 移除固定的100%宽度，让其在flex布局中自适应 */
    /* width: 100%; */
}
...
#instance-chat-footer #instance-chat-input::placeholder {
    color: var(--text-color);
    opacity: 0.5;
}
        /* === 新增：副本氛围设置 - 音乐列表样式 === */
...
.create-group-popup-box .button-group {
    ...
    .diary-settings-section + .diary-settings-section::before {
        content: '';
        position: absolute;
        top: -15px;
        left: 20%;
    ...
    /* 增加板块之间的间距 */
    .diary-settings-section + .diary-settings-section {
        margin-top: 25px;
    }
    ...
    .preset-checkbox-item input[type="checkbox"] {
        ...
        #diary-detail-back-btn { /* 返回按钮 */
             background: none; border: none; cursor: pointer; padding: 4px; z-index: 1; margin-top: 2px;
        }
                ...
                <div class="page" id="page-1">
                    <div class="bento-grid">
                        ...
                        <div class="glass-card area-mid-right" style="position: relative; overflow: visible; background: transparent; border: none; box-shadow: none; backdrop-filter: none; -webkit-backdrop-filter: none; padding: 0;">
                            <!-- 底层：图片上传显示框 (位置和大小保持不变) -->
                            <div id="photo-upload-container" style="position: absolute; z-index: 1; top: 41%; left: 50%; transform: translate(-50%, -50%); width: 97%; height: 72%; background-color: rgba(255, 255, 255, 0.4); border: 2px dashed rgba(255, 255, 255, 0.7); border-radius: 16px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s; background-size: cover; background-position: center;">
                                <span id="photo-upload-placeholder-text" style="color: var(--text-color); opacity: 0.7; font-size: 12px; pointer-events: none;">点击上传图片</span>
                                <input type="file" id="photo-upload-input" accept="image/*" hidden>
                            </div>
                            <!-- 顶层：卡片.PNG 框架 -->
                            <img src="卡片.PNG" alt="卡片框架" style="position: absolute; z-index: 2; top: 45%; left: 50%; transform: translate(-50%, -50%); width: 190%; height: 190%; pointer-events: none;">
                        </div>
                    ...
                    <div class="bento-grid">
                        ...
                        <!-- 3. 第二排右侧：纵向排列的两个App -->
                         <div class="app-grid-area" style="grid-column: 2 / 3; grid-row: 2 / 3; grid-template-columns: 1fr; grid-template-rows: 1fr 1fr; background: transparent; padding: 0 0 0 calc(100% - 80px);">
                            <!-- 正则 -->
                            <div class="app-wrapper" id="app-regex">
                                <div class="app-icon-box">
...
<!-- === Chat App 结构 结束 === -->
    <script>
        ...
        document.getElementById('app-beautify').addEventListener('click', async function() {
            ...
            soundToggle.addEventListener('change', async () => {
                await localforage.setItem('soundEffectsEnabled', soundToggle.checked);
            });
    ...
    async function createCharacterFromData(data, avatarDataUrl) {
        ...
        if (worldBookEntries.length > 0) {
            ...
            if (document.getElementById('modal-title')?.textContent === '世界书') {
    ...
    // ===================================
        ...
        const renderWorldBook = async (expandedCategoryId = null) => {
            // 新增：每次渲染时都从 localforage 重新加载最新的数据，确保数据同步
            const data = await localforage.getItem('worldBookData');
            worldBookData = data ? JSON.parse(data) : [];
            
            const scrollPosition = modalBody.scrollTop; // 保存当前滚动位置
            ...
            worldBookData.forEach((category, index) => {
                ...
                if (isWorldBookEditMode) {
                    ...
                    categoryEl.classList.add('in-edit-mode'); // 禁止长按选择
                    editControlsHTML = `
        ...
        const handleOpenWorldBook = async (e) => {
        ...
        modalBody.addEventListener('click', e => {
            if (modalTitle.textContent !== '世界书') return;
...
<div id="summary-popup-overlay" class="chatapp-popup-overlay">
    <div class="chatapp-popup-box" style="max-width: 480px; gap: 20px;">
        ...
        <div class="modal-form-section" style="gap: 16px;">
            ...
            <div class="modal-form-group">
                <label for="summary-context-slider">总结上下文条数: <span id="summary-context-value">20</span></label>
                <div class="slider-group">
                    <input type="range" id="summary-context-slider" min="10" max="100" step="2" value="20">
                </div>
            </div>
            <div class="modal-form-group">
                <label for="summary-threshold-slider">自动总结阈值: <span id="summary-threshold-value">50</span> 条</label>
                <div class="slider-group">
